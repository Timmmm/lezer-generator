...
export type BuildOptions = {
  ...
  /// `"es"`. Ignored for Typescript output.
  moduleStyle?: "es" | "cjs",
  /// Output code as Typescript. Defaults to false (Javascript).
  typescript?: boolean,
...
class Builder {
  ...
  getParserFile() {
    ...
    let ts = this.options.typescript === true

    let mod = ts ? "es" : (this.options.moduleStyle || "es")
...
${encodeArray(spec.end.compile().toArray({}, none))}, ${spec.placeholder.id}]`
    ...
    head += `export const nodeNames = ${JSON.stringify(nodeNames)}`
    if (ts) {
       head += ` as const
